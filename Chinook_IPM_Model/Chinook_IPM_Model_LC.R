...
temp_df <- read.csv("Egg Modeling Test\\MarshCreek_Temp_2015-2016.csv",header=TRUE) #takes csv file and creates dataframe
...
#  if(max(develop_df$TotalDevelopment,na.rm=T) >= 1))
...
avec = seq(1e-20, 2, length.out = 200) # domain for the larval stage
da = avec[3] - avec[2]
...
#  for(i in 1:200) { #there should be no eggs and no juveniles
 t=200;   for(i in 1:t) { #most of the eggs should have hatched and transitioned to juveniles
  ...
  # Egg development rate -- only calculate this if you are in the spawn window, which is the only time DevelopmentFunc & EmergenceFunc are defined
  ...
  # To compute new juveniles in next stage of time step, we collect those who were
  ...
  if(egg.rate > 0) {
    ...
    egg.dist = egg.dist/sum(egg.dist) # normalizing
      ...
      # Lisa: "adds noise to the development rate in the prediction of the probability of progressing from age class 1 to 2"
...
plot(Eggs ~ times,xlim=c(200,300)) #these seem to be new eggs instead of cumulative eggs
if(t>215) plot(emergence ~ times[1:t],xlim=c(200,300))
if(t>215) range(emergence[emergence>1])
round(Egg.B,2)
 #first egg transitions to a juvenile on day 422, there should be 50K eggs