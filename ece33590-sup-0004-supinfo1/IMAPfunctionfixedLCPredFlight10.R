...
# This version of the IMAPfunction outputs a vector of the number of cumulative flown beetles 
# The output vector runs from June 15 to October 31.

# This version simulates phenology and egg, pupae, teneral adult, and 
# adult mortality only (no larval mortality).

# I call it IMAPfunctionLCPredFlight. LC is for life cycle prediction and Flight.

# Here I take my phenology and mortality model and turn it into a function that can be applied in a loop
# This version differs from the original (Imapfunction.R) in that instead of taking as input mean air 
...
# temperatures (Tmax), It also takes a vector of the density or number
# of individuals laying eggs from June 14 (the first possible recorded
# start time because it's when trapping begins in Alberta). I call this
# a forcing because it is a time-dependent function.

# The ImapFuncLC function returns a vector of:
# the predicted cumulative number beetles
# that have flown.
# Note also that for this version, Tmin and Tmax
# must be exactly 504 days in length 
# (terminating Oct. 31 in normal years and
# Oct. 30 in leap years). 
ImapFuncLC = function(Tmin, Tmax, Forcing){
  #########################################################################################
  ...
  for(i in 2:length(Tmin)){
    ...
    # and each female lays and initial clutch of 82 eggs.
    Fec[i] = 0.5*82*Forcing[i] + Fec[i-1]*exp(-Y1)
    ...
    # If it's warm enough, all previous adults fly.
    # Before I was using mean temperature here but I think
    # maximum temperature is more biologically appropriate.
    if(Tmax2[i] >= 18.3){
      Adults[i] = NewA
    }else{
    ...
    #########################################################
    # Here we keep track of adults that have flown
    ########################################################
    
    # In this version I account for the settling rate of flying beetles.
    if(Tmax2[i] >= 18.3){
      Flying[i] = Flying[i-1]*exp(-0.19*24*1) + Adults[i-1]
    }
    
  } # this ends the i-loop
  
  return(Flying[365:504])
}

# Now I will test the Imap function
# Reading in some temperature data (in degrees C)
#MaxT2 = read.csv("MaxDailyT08to16JunOctVal.csv", h = T)
#MinT2 = read.csv("MinDailyT08to16JunOctVal.csv", h = T)
#MaxT2 = MaxT2[,-1]
#MinT2 = MinT2[,-1]

#Tmin1 = as.numeric(MinT2[531,7:510])
#Tmax1 = as.numeric(MaxT2[531,7:510])

# I make up a forcing fector
#Forcing1 = rep(0,length(Tmin1))
#Forcing1[31] = 10
#Forcing1[32] = 20
#Forcing1[33] = 30
#Forcing1[34] = 20
...
#ptm <- proc.time()
#Out1 = ImapFuncLC(Tmin = Tmin1, Tmax = Tmax1, Forcing = Forcing1)
#proc.time() - ptm

#length(Out1)
#Times = 1:length(365:504)
#plot(Out1 ~ Times)
# Yes; it works.